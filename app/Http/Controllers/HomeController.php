...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::user()->isAdmin()) {
            ...
            return view('home')->with('new_hires', User::allExceptSuperAdmin()->orderBy('prod_date', 'DESC')->paginate(5))->with('employees', User::allExceptSuperAdmin()->get())->with('birthdays', User::whereRaw('MONTH(birth_date) = '.date('n').' && YEAR(birth_date) = '.date('Y'))->get())->with('engagements', ElinkActivities::all());
        }
    }
